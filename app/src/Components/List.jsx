...
function List({ setSelectedMenu, selectedItem, setSelectedItem }) {
  ...
  useEffect(() => {
    try {
      axios.get("http://localhost:5000/media/get-medias").then((responce) => {
        if (responce) {
          console.log(responce.data.medias);
          setDataSource(responce.data.medias);
        }
      });
    } catch (error) {
      console.log(error);
  ...
  const handleConfirm = () => {
    if (dialogType === "delete") {
      ...
      try {
        axios
          .delete(`http://localhost:5000/media/delete-media/${selectedItem.id}`)
          .then((responce) => {
  ...
  return (
    <div className=" overflow-hidden max-h-[90vh]">
      ...
      <div className="max-h-[80vh] overflow-y-auto border border-gray-300 rounded-lg mt-0">
        <table className="table-auto border-collapse border  overflow-y-auto   border-gray-300 w-full min-h[70%] text-left">
          ...
          <tbody>
            {dataSource.length > 0 ? (
              dataSource.map((media) => (
                <tr key={media.id} className="font-bold">
                  <td className="border px-4 py-2">
                    <img src={media.image} className="w-40 " />
                  </td>
